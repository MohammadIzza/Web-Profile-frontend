      ...
      {blogs.map((blog) => (
        <Card key={blog.id} className="bg-paper border-line hover:brightness-95 transition duration-200 group flex flex-col max-h-[320px] sm:max-h-[380px] lg:max-h-[420px]">
          ...
          <CardHeader className="p-3 sm:p-4 lg:p-6 pb-2 sm:pb-3 flex-shrink-0">
            ...
            <CardTitle className="text-sm sm:text-base lg:text-lg font-serif font-semibold text-ink line-clamp-2 group-hover:text-ink/80 transition-colors">
              {blog.title}
            </CardTitle>
            {blog.excerpt && (
              <CardDescription className="text-xs sm:text-sm font-serif text-ink/60 line-clamp-2 sm:line-clamp-3 mt-1 sm:mt-2">
                {blog.excerpt}
              </CardDescription>
            )}
          </CardHeader>
          <CardContent className="p-3 sm:p-4 lg:p-6 pt-0 flex-1 flex flex-col justify-end">
            ...
            <Separator className="mb-2 sm:mb-4 bg-line" />
            <Button 
              variant="ghost" 
              size="sm" 
