...
export default function TechStackSection({ techStacks, loading }: TechStackSectionProps) {
  ...
  const categoryCards = ['frontend', 'backend', 'tools', 'database', 'devops', 'other']
    .map((category) => {
      ...
      return (
        <Card key={category} className="bg-paper border-line flex-shrink-0 w-[280px] sm:w-[320px] lg:w-[360px]">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg font-serif font-semibold text-ink capitalize">
              {category}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {categoryTechs.map((tech) => (
                <div key={tech.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-base font-serif font-medium text-ink/80">
                      {tech.name}
                    </span>
                  </div>
